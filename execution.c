...
void execute_acc(Instruction, Processor *);
...
void execute_instruction(uint32_t instruction_bits, Processor *processor,Byte *memory) {    
    ...
    switch(instruction.opcode) {
        case 0x33:
            ...
            break;
        case 0x0b:
            execute_acc(instruction, processor);
            break;
        case 0x13:
            ...
            switch (instruction.rtype.funct7) {
                ...
                case 0x20:
                    ...
                    processor->R[instruction.rtype.rd] = 
                        ...
                        (sWord)processor->R[instruction.rtype.rs2]);   
                    processor->R[instruction.rtype.rd] = 
                        sign_extend_number(processor->R[instruction.rtype.rd], 
                        32 - processor->R[processor->R[instruction.rtype.rs2]]);
                    break;
                default:
                    handle_invalid_instruction(instruction);
                    exit(-1);
...
void execute_itype_except_load(Instruction instruction, Processor *processor) {
    switch (instruction.itype.funct3) {
        ...
        case 0x5:
            ...
            if((processor->R[instruction.itype.imm] & 0b10000000000) == 1)    {
...
void execute_acc(Instruction instruction, Processor *processor) {
    processor->R[instruction.rtype.rd] += ( 
    ...
    switch (instruction.itype.funct3) {
        case 0x0:
            ...
            processor->R[instruction.itype.rd] = 
                load(memory, 
                    (sWord)processor->R[instruction.itype.rs1] + 
                    (sWord)sign_extend_number(instruction.itype.imm, 12), LENGTH_BYTE);
            break;
        case 0x1:
            // LH
            processor->R[instruction.itype.rd] = 
                load(memory, 
                    (sWord)processor->R[instruction.itype.rs1] + 
                    (sWord)sign_extend_number(instruction.itype.imm, 12), LENGTH_HALF_WORD);
            break;
        case 0x2:
            // LW
            processor->R[instruction.itype.rd] = 
                load(memory, 
                    (sWord)processor->R[instruction.itype.rs1] + 
                    (sWord)sign_extend_number(instruction.itype.imm, 12), LENGTH_WORD);
            break;
        default:
            handle_invalid_instruction(instruction);
            break;
